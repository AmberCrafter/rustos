...
pub fn sys_read(buffer: *mut u8, len: usize) -> isize {
    ...
    loop {
        without_interrupts(|| {
            if let Some(ch) = STDIN_BUFFER.lock().pop_front() {
                c = ch;
            }
        });
...
pub fn sys_exit(exit_code: isize) -> ! {
    use crate::library::processor::exit_current_and_run_next;
    serial_println!("[kernel] Task exited with return code {}", exit_code);
    exit_current_and_run_next(exit_code);
    panic!("sys_exit never return");
}

pub fn sys_fork() -> isize {
    use crate::library::processor::{current_process, manager::add_process};
...
pub fn sys_exec(app_name: *const u8) -> isize {
    use crate::library::loader::get_app_data_by_name;
    use crate::library::processor::{current_process, manager::add_process};
    let length: usize;
